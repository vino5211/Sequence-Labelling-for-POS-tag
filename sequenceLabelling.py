...
def main():
    ...
    if args.do_test:
        ...
        args = torch.load(os.path.join(args.output_dir, 'training_args.bin'), weights_only=False)
        ...
        with open(os.path.join(args.output_dir, "token_labels_.txt"), "w", encoding="utf-8") as f:
            for ori_tokens, ori_labels,prel in zip(all_ori_tokens, ori_labels, pred_labels):
